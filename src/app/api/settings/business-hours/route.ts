...
//
//   GET — the account's timezone + a room's weekly business hours. Any member.
//   PUT — replace the whole weekly schedule for a room + timezone.  Admin+.
//
// Horario de clínica por consultorio. El slot logic de la reserva pública
// (src/lib/scheduling/business-hours.ts) lee estas filas para acotar la
// disponibilidad. PUT es un reemplazo total de las filas de ESE consultorio
// para que la UI pueda mandar la semana entera en una sola llamada.
// ============================================================
...
export async function GET(request: Request) {
  try {
    ...
    // Horario por consultorio: sin room_id no hay filas que devolver.
    let days: unknown[] = [];
    if (roomId) {
      const { data: hours } = await ctx.supabase
        .from("business_hours")
...
export async function PUT(request: Request) {
  try {
    ...
    const body = (await request.json().catch(() => null)) as
      | { timezone?: unknown; days?: unknown; room_id?: unknown }
      | null;
    if (!body) return NextResponse.json({ error: "Invalid request body" }, { status: 400 });

    // Horario por consultorio: el room_id es obligatorio y debe ser de esta cuenta.
    ...
    for (const raw of body.days) {
      ...
      if (d.close_time <= d.open_time) {
        return NextResponse.json({ error: "close_time must be after open_time" }, { status: 400 });
      }
    ...
    // Reemplazo total de las filas DE ESTE consultorio. RLS acota ambas
    // sentencias a la cuenta (business_hours_modify → admin).
    ...
    if (rows.length > 0) {
      const { error: insErr } = await ctx.supabase
        .from("business_hours")
        .insert(rows.map((r) => ({ ...r, account_id: ctx.accountId, room_id: roomId })));
      if (insErr) {
