...
/**
 * Clinic-hours helper. Turns a room's weekly `business_hours` rows (local
 * wall-clock open/close per weekday) into concrete UTC `TimeRange`s for a
 * given window, honoring the account's IANA timezone (`accounts.timezone`,
 * default America/Mexico_City).
 *
 * Pure timezone math via Intl — no external date library. Mexico no longer
 * observes DST (America/Mexico_City is a fixed offset since 2023), but the
 * single-pass offset lookup below is correct for any wall-clock time that
 * isn't inside a DST spring-forward gap, which is good enough for clinic
 * opening hours.
 */
...
/**
 * Public helper: interpret a "YYYY-MM-DD" date + "HH:MM" wall-clock time
 * as local time in `tz` and return the UTC instant as ISO. Used when staff
 * declare a doctor's availability block in clinic-local time so it lines up
 * with clinic hours.
 */
 ...
 * `configured: false` with no ranges when the room has no business hours —
 * callers treat that as "no clinic-hours constraint" (fall back to the
 * doctor's own declared blocks).
 */
export async function computeClinicRanges(
  ...
  // Horarios POR CONSULTORIO: sin roomId no hay horario de clínica que
  // aplicar (se cae al comportamiento de bloques del médico).
  if (!roomId) return { ranges: [], configured: false };
