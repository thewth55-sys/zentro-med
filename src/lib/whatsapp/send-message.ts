...
export async function sendMessageToConversation(
  ...
  // API + QR, see migration 115), and a reply has to go back out the
  // same line the contact is talking to (migration 116). Solo para
  ...
  if (!isDemo) {
    ...
    if (!cfg && !configError) {
      ...
      // column (migration 116 backfills these, but a row created in the
