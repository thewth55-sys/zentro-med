 ...
 * passes (it's the super-admin role, preserved for today's staff by
 * migration 147's default), everyone else needs their `role` to be
 * in `allowed`. Only 'marketing' has a real caller today; the other
 * 4 roles (support/dev/qa/customer_success) are assignable via
 * /admin/team but have no enforced permissions yet.
 */
