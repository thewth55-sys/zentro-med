-- ============================================================
-- 089_conversation_response_reminders.sql — recordatorios ESCALADOS para
-- conversaciones entrantes sin responder.
--
-- Cuando un cliente escribe y nadie del equipo responde, se dispara un
-- recordatorio a los 5 min, luego 10, 20, 40 y de ahí cada 60 min (hora),
-- hasta que un usuario responda o se cierre la conversación.
--
-- Mecánica:
...
--   - El cron (/api/conversation-reminders/cron) lee las filas vencidas,
--     aplica la curva de escalado, verifica el plan de la cuenta (feature
--     premium) e inserta una fila en `notifications` por destinatario
...
-- Los miembros de la cuenta pueden LEER el estado (depuración / UI futura).
-- Las escrituras son exclusivas de los triggers SECURITY DEFINER y del cron
...
ALTER TABLE notifications ADD CONSTRAINT notifications_type_check
  CHECK (type IN ('conversation_assigned', 'response_reminder'));
